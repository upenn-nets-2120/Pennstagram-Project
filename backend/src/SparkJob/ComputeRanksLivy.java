...
public class ComputeRanksLivy {
    ...
    public static void main(String[] args)
        ...
        SocialRankJob noBackLinks = new SocialRankJob(d_max, i_max, 1000, false, debug);

        List<MyPair<String, Double>> noBacklinksResult = SparkJob.runJob(livy, noBackLinks);
        ...
        try (PrintStream out = new PrintStream(new FileOutputStream("socialrank-livy-results.txt"))) {
            out.println("With backlinks: " + backlinksResult);
            out.println("Without backlinks: " + noBacklinksResult);
        } catch (Exception e) {
            logger.error("Error writing to file: " + e.getMessage());
        }
