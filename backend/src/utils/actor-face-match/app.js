...
async function initializeFaceModels() {
  ...
  await tf.ready();
  await faceapi.nets.ssdMobilenetv1.loadFromDisk('../utils/actor-face-match/model');
...
/**
 * Example: Main function to find top matching faces and return them in a structured format
 * 
 * @param {string} profilePhoto The filename of the image to be searched
 * @returns {Promise<Array>} A promise that resolves to an array of match results
 */
async function main(profilePhoto) {
  const client = new ChromaClient();
  await initializeFaceModels(); // Ensure models are initialized before proceeding
  ...
  try {
      ...
      return formattedResults; // Return the results in an array of objects
  } catch (err) {
      console.error("Error during face matching:", err);
      throw err; // Rethrow the error to handle it outside if necessary
  }
};
