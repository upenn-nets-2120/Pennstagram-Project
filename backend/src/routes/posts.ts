...
posts.put('/:postID/update', async (req: Request, res: Response) => {
    ...
    const post = await db.query('SELECT * FROM posts WHERE postID = ?', postID);
    ...
    await db.query('UPDATE posts SET ? WHERE postID = ?', [updatedPost, postID]); //SET caption, hashtag to some new value
...
posts.post('/:postID/like', async (req: Request, res: Response) => {
    ...
    await db.query('INSERT INTO likes SET postID = ?, liker = ?', [postID, userID]);
